...
// https://github.com/marcglasberg/assorted_layout_widgets/blob/master/lib/src/wrap_super.dart
enum WrapType {
...
class _RenderWrapSuper extends RenderBox
  ...
  @override
  double computeMaxIntrinsicHeight(double width) => computeDryLayout(BoxConstraints(maxWidth: width)).height;
  ...
  void performLayoutElse() {
    ...
    // Will try to minimize the difference between line widths.
    if (wrapType == WrapType.balanced) {
      List<List<num>> result = MinimumRaggedness.divide(intrinsicWidths, availableWidth, spacing: spacing);
      lines = result.map((List<num> indexes) => _Line()..indexes = indexes as List<int>).toList();
    }
    //
    // Will fit all widgets it can in a line, and then move to the next one.
  ...
  _Result _calculateForWrapFitLarger(
    ...
    for (_Line line in lines) {
      ...
      var preferredWidthForRemainingWidgets = remainingWidth / (line.indexes.length - numberOfLargerWidgets);
      ...
      for (int index in line.indexes) {
        ...
        if (childWidth >= preferredWidth) {
          if (childWidth > availableWidthMinusSpacing) {
            childConstraints =
                BoxConstraints(minWidth: availableWidthMinusSpacing, maxWidth: availableWidthMinusSpacing);
          } else
        ...
        // 3) Divide the remaining width by the remaining widgets.
          ...
          childConstraints =
              BoxConstraints(minWidth: preferredWidthForRemainingWidgets, maxWidth: preferredWidthForRemainingWidgets);
        }
