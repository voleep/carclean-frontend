...
import 'package:voleep_carclean_frontend/shared/widgets/search_form/domain/enums/filter_date_enum.dart';
...
class FilterDateView extends ConsumerWidget {
  ...
  DateTime _getValueByDate(FilterDateEnum filterDate) {
    ...
    if (filterDate == FilterDateEnum.lastMonth) {
      ...
      return DateTime(firstDayOfLastMonth.year, firstDayOfLastMonth.month,
  ...
  DateTime _getFinalValueByDate(FilterDateEnum filterDate) {
    ...
    if (filterDate == FilterDateEnum.lastMonth) {
      ...
      return DateTime(lastDayOfLastMonth.year, lastDayOfLastMonth.month,
  ...
  String _getDateLabelByValue({required DateTime value, DateTime? finalValue}) {
    ...
    if (finalValue == null ||
        value.add(const Duration(days: 1)) == finalValue) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final queryDateList = ref.watch(filterQueryProvider(config).select(
        (value) =>
            value
              ...
              if (value == true) {
                ref.read(filterQueryProvider(config).notifier).add(
                      FilterQueryState(
                        ...
                        valueLabel: _getDateLabelByValue(
                            value: _getValueByDate(filterDate),
                            finalValue: _getFinalValueByDate(filterDate)),
                        value: _getValueByDate(filterDate),
                        finalValue: _getFinalValueByDate(filterDate),
          ...
          children: [
            TextButton(
              ...
              onPressed: () async {
                ...
                if (selectedDate != null) {
                  final DateTime dayAfterSelected =
                  ...
                  ref.read(filterQueryProvider(config).notifier).add(
                        FilterQueryState(
                          ...
                          value: DateTime(selectedDate.year, selectedDate.month,
                                  selectedDate.day)
                              .toString(),
                          finalValue: DateTime(dayAfterSelected.year,
                                  dayAfterSelected.month, dayAfterSelected.day)
                              .toString(),
                        ),
                      );
                }
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                if (selectedDate != null) {
                  final finalDate =
                      selectedDate.end.add(const Duration(days: 1));
                  ...
                  ref.read(filterQueryProvider(config).notifier).add(
                        FilterQueryState(
                          ...
                          valueLabel: _getDateLabelByValue(
                              ...
                              finalValue: selectedDate.end),
                          value: DateTime(
                                  selectedDate.start.year,
                                  selectedDate.start.month,
                                  selectedDate.start.day)
                              .toString(),
                          finalValue: DateTime(finalDate.year, finalDate.month,
                                  finalDate.day)
                              .toString(),
                        ),
                      );
