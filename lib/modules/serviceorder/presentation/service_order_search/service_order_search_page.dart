...
class ServiceOrderSearchPage extends ConsumerWidget {
  ServiceOrderSearchPage({super.key});

  final _searchConfig = SearchConfig(endpoint: "${ApiConfig.CARCLEAN_API_URL}/serviceorder", orderField: "description", filterOnInit: true);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      body: SafeArea(
        child: CarCleanSearch<ServiceOrderModel>(
          ...
          itemBuilder: (context, index, item) => ListTile(
            ...
            onTap: () async {
              final shouldReload = await context.push(
                Routes.app.serviceOrder.update(item.serviceOrderId),
              );
          ...
          fromJsonT: ServiceOrderModel.fromJson,
        ),
      ),
      floatingActionButton: FloatingActionButton(
          child: const Icon(Icons.add_rounded),
          onPressed: () async {
            final shouldReload = await context.push(Routes.app.serviceOrder.create);
            if (shouldReload == true) {
              ref.read(searchControllerProvider(_searchConfig).notifier).refresh();
            }
          }),
